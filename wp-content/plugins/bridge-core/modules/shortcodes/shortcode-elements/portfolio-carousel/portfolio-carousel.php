...
namespace Bridge\Shortcodes\PortfolioCarousel;

use Bridge\Shortcodes\Lib\ShortcodeInterface;

class PortfolioCarousel implements ShortcodeInterface {
	private $base;
	
	public function __construct() {
		$this->base = 'qode_portfolio_carousel';
		
		add_action( 'vc_before_init', array( $this, 'vcMap' ) );
		
		//Portfolio category filter
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_category_callback', array( &$this, 'portfolioCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio category render
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_category_render', array( &$this, 'portfolioCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects filter
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_selected_projects_callback', array( &$this, 'portfolioIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects render
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_selected_projects_render', array( &$this, 'portfolioIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
		
		//Portfolio tag filter
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_tag_callback', array( &$this, 'portfolioTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio tag render
		add_filter( 'vc_autocomplete_qode_portfolio_carousel_tag_render', array( &$this, 'portfolioTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Portfolio Carousel', 'bridge-core' ),
					'base'     => $this->getBase(),
					'category' => esc_html__( 'by QODE', 'bridge-core' ),
					'icon'     => 'icon-wpb-portfolio-carousel extended-custom-icon-qode',
					'params'   => array(
						array(
							'type'        => 'dropdown',
                        ...
                        array(
                            ...
                            'heading'     => esc_html__( 'Enable Carousel Auto Width', 'bridge-core' ),
                            'value'       => array_flip( bridge_qode_get_yes_no_select_array(false) ),
                            'save_always' => true
                        ),
						array(
	...
	public function render( $atts, $content = null ) {
		...
		$html = bridge_core_get_shortcode_template_part( 'templates/portfolio-carousel-holder', 'portfolio-carousel', '', $params );
